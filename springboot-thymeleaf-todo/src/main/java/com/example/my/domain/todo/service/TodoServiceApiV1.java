...
public class TodoServiceApiV1 {
    ...
    public ResponseEntity<?> updateTodoTableData(Long todoIdx, ReqTodoTableUpdateDoneYnDTO dto,
        ...
        // 할 일이 null이면 (존재하지 않는 할 일입니다.) 리턴
        if (!todoEntityOptional.isPresent()) {
            throw new BadRequestException("존재하지 않는 할 일입니다");
        }

        // 할 일 작성자와 로그인 유저가 다르면 (권한이 없습니다. )리턴
        if (todoEntityOptional.get().getUserEntity().getIdx() != loginUserDTO.getUser().getIdx()) {
            throw new BadRequestException("권한이 없습니다");
        }

        // 할 일 doneYn 업데이트
        todoEntityOptional.get().setDoneYn(dto.getTodo().getDoneYn());
    ...
    public ResponseEntity<?> deleteTodoTableData(Long todoIdx, LoginUserDTO loginUserDTO) {
        ...
        // 할 일 deleteDate 업데이트
        todoEntityOptional.get().setDeleteDate(LocalDateTime.now());
